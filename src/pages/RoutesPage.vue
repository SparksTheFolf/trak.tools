          ...
          <div class="col-md-2 col-sm-4 col-6 mb-2" v-for="(filter, key) in routeTypeFilters" :key="key">
            <div class="form-check">
              ...
              <label class="form-check-label" :for="`filter-${key}`">
                ...
                {{ filter.label }}
                <span class="badge bg-secondary ms-1">{{ getRouteCountByType(Number(key)) }}</span>
              </label>
            </div>
          </div>
        </div>
...
const updateFilteredRoutes = () => {
  filteredRoutes.value = routes.value.filter((route: any) => {
    ...
    return routeTypeFilters.value[routeType as keyof typeof routeTypeFilters.value]?.enabled || false
  })
}
...
const getRouteType = (type: number): string => {
  return routeTypes[type as keyof typeof routeTypes] || 'Unknown'
}
...
const selectAllFilters = () => {
  Object.keys(routeTypeFilters.value).forEach(key => {
    routeTypeFilters.value[key as unknown as keyof typeof routeTypeFilters.value].enabled = true
  })
  updateFilteredRoutes()
}
...
const clearAllFilters = () => {
  Object.keys(routeTypeFilters.value).forEach(key => {
    routeTypeFilters.value[key as unknown as keyof typeof routeTypeFilters.value].enabled = false
  })
  updateFilteredRoutes()
}

const toggleRawJson = (routeId: string) => {
  const route = routes.value.find((r: any) => r.id === routeId) || null
